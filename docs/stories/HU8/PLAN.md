...
A pure function `detect_housing_benefits` in `scoring_engine/housing_benefits.py` evaluates eligibility rules for all six benefits against the existing profile data. It runs after the scoring pipeline and appends a `housing_benefits` object to the result. The frontend reads this field and renders a "Simulación" section in `Recommendations.jsx` (next to "Plan de Mejora") with benefit details, contextual Academy links, and a disclaimer. No score modification, no new form fields, no migration.
...
No ALG amendment. Detection rules per benefit:
...
6. Update `frontend/src/components/Recommendations.jsx`: move benefits into "Simulación" section next to Plan de Mejora, add Academy links per benefit, use exact disclaimer text.
