...
Para el desarrollo del MVP de ScoreLeads se propone una arquitectura web simple, separando frontend, backend y persistencia de datos. En el frontend se utilizará **React**, ya que permite construir interfaces dinámicas de forma rápida, como formularios guiados, visualización del score y recomendaciones personalizadas. Se prefiere React sobre Angular para el MVP debido a su mayor rapidez para prototipar.
...
Se realizaron ajustes en las Historias de Usuario 1, 2 y 3 con el objetivo de alinear mejor el MVP de ScoreLeads con una propuesta más atractiva tanto al usuario final como al ejecutivo comercial.
...
**Dado** que el usuario ha completado todos los campos del formulario, **cuando** intenta enviar sus datos sin haber aceptado el consentimiento explícito, **entonces** el sistema bloquea el envío y resalta el campo de consentimiento indicando que es obligatorio.

**E4 - Complemento de renta:**
**Dado** que el usuario desea complementar su renta con otra persona para mejorar su capacidad de compra, **cuando** complete el formulario y seleccione que realizará el proceso con un complemento, **entonces** el sistema debe solicitar los datos financieros básicos del complementario e incorporarlos al análisis preliminar.
...
**Dado** que el sistema de ScoreLeads recibió los datos del Usuario, **cuando** se realiza la calificación, **entonces** el resultado debe clasificar al usuario en niveles de prioridad claros (ej: Alto, Medio, Bajo).
...
**Dado** que el usuario recibe un scoring, **cuando** el resultado es generado correctamente, **entonces** el sistema debe guardar la fecha, score, clasificación y explicación asociada para mantener trazabilidad del historial.
