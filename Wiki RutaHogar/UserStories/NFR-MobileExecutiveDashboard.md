# NFR — Mobile-Adaptable Executive Dashboard

> ℹ️ **Non-functional requirement.** The updated sprint plan ("HUs para Sprint 1" document) lists the executive mobile dashboard as a non-functional requirement ("Dashboard móvil ejecutivo") rather than a functional user story. Kept here for reference; it was `HU 30` under the E4 plan.

A card-based, responsive version of the executive lead dashboard, so executives can review, filter, and act on leads from a phone with the same permissions as desktop.
...
- Mobile counterpart of [[HU2-LeadPrioritization\|HU 2]]; the lead-side counterpart is [[NFR-MobileLeadExperience\|Mobile Lead Experience (NFR)]].
- E4 quick actions (change status, report inconsistency) connect to [[HU17-FraudulentLeadReporting\|HU 17]] and the lead status model.
- E6 role enforcement aligns with [[NFR-BasicSecurity\|Basic Security (NFR)]] and [[NFR-RolesAndPermissions\|Roles & Permissions (NFR)]].
